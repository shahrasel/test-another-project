<link rel="stylesheet" href="<?php echo getConfigValue('base_url') ?>css/general.css" type="text/css" media="screen" />
<script language="javascript" src="<?php echo getConfigValue('base_url') ?>js/popup.js"></script>
...
function loadpopupnotification(id) {
	document.getElementById('formdiv').innerHTML = "<form action='<?php echo urlForAdmin($module.'/sendnotification') ?>' method='post'><div style='width: 490px; float: left; padding-left: 80px;padding-top: 20px;padding-bottom: 20px;'>Send Push Notification</div><div style='width: 490px; float: left; padding-top: 20px; padding-left: 80px;padding-bottom: 20px;'><textarea name='mtext' cols='50' rows='5'></textarea><br/><br/><input type='hidden' id='hiddenval' name='hiddenval' value=''><input type='submit' value='Send Notification'></div></form>";
	centerPopup();
	loadPopup();
...
?>
<div style="width:935px;float:left;position:relative;color:#F00;text-align:center"><?php echo getSessionValue('sess_status');  setSessionValue('sess_status','') ?></div>
<?php 
...
<div class="product">
  ...
  <div class="product_2">
    ...
    <div style="float:left">Push Notification</div>
...
<div class="task">
  ...
  <div class="task_2" style=" overflow:auto;">
     ...
     <?php 
      ...
      <?php  
      if ( getSessionValue('sent_message') != '' ): ?>
      <div align="center">
        <div class="<?php echo ( !empty($err)?'err_div':'msg_div');?>" >
          <?php echo  getSessionValue('sent_message'); unsetSessionValue('sent_message'); ?>
        </div>
      </div>
      <?php endif?>
    ...
    <table width="895" border="0" align="left" >
      ...
      <tr class="tr<?php echo ($tmp_count%3)?>">
        ...
        <?php endforeach; ?>
        <td class="flash_03">
        	<a class="featuredcrop" onclick="loadpopupnotification('<?php echo $row_data['id'] ?>')" href="#">Notify</a>
        </td>
        <td class="flash_03">
        	<a href="<?php echo urlForAdmin($module.'/manage')?>?id=<?php echo $row_data[id]?>&delete=yes" onclick="return deleteConfirmation();"><img src="<?php echo getConfigValue('media_url')?>images/b_drop.png" border="0"></a>
        </td>
      ...
      <tr >
        ...
        <td colspan="<?php echo count($table_row_column)+3?>" align="left"  class="flash_03">
          ...
          <input type="button" value="Execute All" onclick="return processConfirmation('<?php echo $module.'form'?>');" /></td>
      </tr>
