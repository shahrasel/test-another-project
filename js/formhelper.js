...
function checkTextArea( inputid, warning ){
	alert('shihab');
	if( warning == '' ){
		warning = 'Required';
	}
...
var file_no = 2;
function add_more_files(id, style_class, to_id){
	//alert (id)
	upload_html = '<div class="gallery_box_row" id="gallery_box_row_'+file_no+'" style="width:500px; float:left;">'+
										'<div class="gallery_box_input" style="float:left; width:250px; padding-top:5px;"><input type="file" name="'+id+'_'+file_no+'" id="'+id+'_'+file_no+'" class="'+style_class+'" /> </div>'+
										'<div class="file_remove" style="float:left; width:200px; padding-top:5px; padding-left:5px;" onclick="remove_more_files(\'#gallery_box_row_'+file_no+'\');"><img src="/amarboimela/images/btn_delete.png" border="0"/></div>'+
								 '</div>';
										
	$("#"+to_id).append(upload_html);
		
	file_no++;
}

function remove_more_files(t_id){
	$(t_id).remove();
}
...
function processConfirmation(myform){
	//alert (myform);
	var where_to= confirm("Do you really want to this batch process?");
