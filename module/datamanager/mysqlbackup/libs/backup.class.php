...
class backup extends db{
	...
	function backup(){
		global $GonxAdmin;
		if (empty ($GonxAdmin) )
			$GonxAdmin = getConfigValue ('GonxAdmin');
...
//					$contents = bzread($bz, filesize($this->backupdir."/".$bfile)*1000); // just a hack coz feof doesn't wrk for me
	...
	function listbackups(){
		...
		if (empty ($GonxAdmin) )
			$GonxAdmin = getConfigValue ('GonxAdmin');
		if (empty ($page) )
			$page = getConfigValue ('page');
		if (empty ($orderby) )
			$orderby = getConfigValue ('orderby');	
...
\t<tr>
			...
			<td align=center><a href=\"?option=databaseAdmin&go=delete&fname=".$v['fname']."\" title=\"".$GONX["delete"]." ".$v['fname']."\" onclick=\"return IECColor2($k);\"><img src=\"".getConfigValue('base_url').'media/images/b_drop.png'."\" border=0></a></font><td>
</tr>\n\n";
...
<input type=hidden name=go value=saveconfig>
	...
	function getbackup($bfile){
		...
		if (is_file($this->backupdir."/".$bfile) and !ereg("../",$bfile)) {
			...
			$path_parts = pathinfo($this->backupdir."/".$bfile);
						
			if ($path_parts['extension'] == 'gz' )
				header("Content-Type: application/x-gzip");
			else if ($path_parts['extension'] == 'bz2')
			...
			//header("Content-Type: application/x-gzip");
			header("Content-Disposition: attachment; filename=".$bfile);
